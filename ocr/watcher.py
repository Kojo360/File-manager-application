...
TESSERACT_CMD = r"C:\Users\KC-User\AppData\Local\Programs\Tesseract-OCR\tesseract.exe"  # Adjust if needed
...
def route_file(src_path):
    ...
    # Determine new filename and destination
    if name and account:
    ...
    if is_image:
        try:
            ...
            os.remove(src_path)
            logging.info(f"[{dest_dir.upper()}] {filename} → {new_filename} (image → PDF)")
        except Exception as e:
...
class ScanHandler(FileSystemEventHandler):
    ...
    def on_modified(self, event):
        # Optional: react to modifications
        pass


def start_watcher():
    logging.info(f"Watching directory: {SCAN_DIR}")
    observer = Observer()
